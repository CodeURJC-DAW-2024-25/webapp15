...
public class OrderItemController {
    ...
    @GetMapping("/addItem")
    public ResponseEntity<String> addItemToCart(@RequestParam Long id_Shoe, @RequestParam String size, @RequestParam int cuantity,Long id_user) {
        ...
        //first verify if the shoe is in the cart, in that case just increment the cuantity
        Optional<OrderItem> orderItemOptional=orderItemService.findByCartAndShoeAndSize(id_user, id_Shoe, size);
        OrderItem oderItem = orderItemOptional.get();
        if(orderItemOptional.isPresent()){//in case that exist in the cart, increase the cunatity
            oderItem.setQuantity(oderItem.getQuantity()+1);
            orderItemService.save(oderItem);
        }else{
            OrderItem newOrderItem=new OrderItem(cart, shoe_optional.get(), cuantity, size);
